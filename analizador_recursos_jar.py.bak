...
def obtener_mapeo_nombres(jar_path):
    """Lee ExportInfo del JAR y arma un mapeo jarentryname -> instanceId con extensión"""
    mapping = {}
    try:
        with zipfile.ZipFile(jar_path, "r") as jar:
            if "ExportInfo" in jar.namelist():
                ...
                # Diccionario para mapear typeId -> extensión
                type_extensions = {
                    "WSDL": ".wsdl",
                    "ProxyService": ".proxy",
                    "Pipeline": ".pipeline",
                ...
                for item in root.findall("imp:exportedItemInfo", ns):
                    ...
                    # Asignar extensión según typeId
...
if archivo:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if servicio_detectado:
            ...
            st.warning("⚠️ No se encontró servicio con carpeta EXP o pipeline asociado.")
            
            # Opción de nombre de servicio manual
        ...
        if st.button("⚡ Ejecutar transformación"):
            ...
            # st.download_button(
                # label="⬇️ Descargar Excel Normal",
