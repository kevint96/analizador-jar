...
if archivo:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if st.button("🚀 Ejecutar transformacion"):
            ...
            # Convertir a HTML y aplicar estilos
            html_table = df_unico.to_html(index=False).replace("\\n", "<br>")
            ...
            # Insertar tabla con clase CSS
            st.markdown(f"<table class='styled-table'>{html_table.split('<table border=\"1\" class=\"dataframe\">')[1]}", unsafe_allow_html=True)
